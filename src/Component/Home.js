...
function Home(props) {
    ...
    const FetchNews = async () => {
        const response = await axios.get(`https://newsapi.org/v2/everything?q=${props.category}&from=${year + '-' + month + "-" + predate}&to=${dates.toLocaleDateString()}&sortBy=popularity&apiKey=dc02ca6ff93e4e90b2681bb1451b2465&page=${page}&pagesize=6`)
        setMyloading(true)
    ...
    const FetchNext = async () => {
        const response = await axios.get(`https://newsapi.org/v2/everything?q=${props.category}&from=${year + '-' + month + "-" + predate}&to=${dates.toLocaleDateString()}&sortBy=popularity&apiKey=7eb3e58aae3d49a381d73fa17ff4364b&page=${page + 1}&pagesize=6`)
        setMypage(page + 1)
        setMyloading(true)
        const json = response.data
                        ...
                        return <div className="col md-4 " key={index}>
                            <div className={`card my-1`} style={{ width: "19rem", height: "14.1rem", overflow: "hidden" }}>
                               <div className="card-body" >
                                    ...
                                    <a href={element.url} target='_blank' rel="noreferrer" className='btn btn-sm btn-primary' style={{ position: "absolute", top: "11.6rem" }}>Read More</a>
                                    {isfavourite?<FavoriteIcon style={{ position: "absolute", left: "14rem", top: "11.6rem", color: 'red', cursor: "pointer" }} />:
                                    <FavoriteBorderIcon onClick={() => handleclid(element.url, element)} style={{ position: "absolute", left: "14rem", top: "11.7rem", cursor: 'pointer' }} />}
                                </div>
                            </div>
                        </div>
                    })}
